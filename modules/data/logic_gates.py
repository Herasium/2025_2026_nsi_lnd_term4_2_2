...
def gate_evaluate(self) -> List[Tuple[int, bool]]:
    ...
    # compute via instance wrapper si disponible
    try:
    ...
    # ajuste la taille de self.outputs si nécessaire
...
def path_propagate_to_connected_gates(self, editor) -> List[Any]:
    """
    Pour chaque connexion sur ce path (inputs+outputs), appelle receive_input
    sur la gate connectée. Retourne la liste d'ids de gates modifiées (marquées dirty).
    Conventions (d'après ton code) : tuple = (flag, gate_id, gate_port_index, ...)
    """
    modified_gates = set()
...
class Simulator:
    ...
    - editor doit exposer .gates (dict id->Gate) et .paths (dict id->Path)
    - step() doit être appelé depuis EditorView.on_update(delta_time)
    """
...
def _inject():
    try:
        ...
        # classes non présentes pour l'instant (importer ce module plus tard)
        return

    # n'injecte que si la méthode n'existe pas (pour ne pas écraser)
...
# injection immédiate si possible
_inject()
